...
ADCSensor sus_travel_fl_sens(ADC3_CS, 3, 0,0); // will need corrected values
ADCSensor sus_travel_fr_sens(ADC3_CS, 0, 0,0); // (sensor is not chosen yet  ??? nice.)

ADCSensor brake_temp_fr_sens(ADC1_CS, 0, 0, 0);
ADCSensor brake_temp_fl_sens(ADC1_CS, 0, 0, 0);
...
void sample_ADCs_F(int &ATCC){

  static EasyTimer sample_timer_1(1000); // 1,000 Hz - static variable prevents object from deletion when out of scope
