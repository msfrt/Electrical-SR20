...
int brightness_sensor(const int &pin, const int analog_read_bits){
  ...
  if (running_sample_count >= sample_count_max){
    ...
    mvoltage = (static_cast<float>(running_total) / static_cast<float>(max_read_val)) * uC_millivoltage;

    // map the brightness to a percentage 0-100
    last_calcd_value = map(mvoltage, nighttime_mVoltage, daytime_mVoltage, 0, 100);

    if (last_calcd_value > 100)
      last_calcd_value = 100;

    if (last_calcd_value < 0)
      last_calcd_value = 0;
    ...
    Serial.print("Light sensor mVoltage: "); Serial.println(mvoltage);
    Serial.print("Mapped percent: "); Serial.println(last_calcd_value);
    #endif
